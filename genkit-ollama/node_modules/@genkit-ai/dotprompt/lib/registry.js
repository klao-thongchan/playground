...
function registryDefinitionKey(name, variant) {
  return `dotprompt/${name}${variant ? `.${variant}` : ""}`;
}
function registryLookupKey(name, variant) {
  return `/prompt/${registryDefinitionKey(name, variant)}`;
}
function lookupPrompt(name, variant, dir = "./prompts") {
  return __async(this, null, function* () {
    const registryPrompt = yield (0, import_registry.lookupAction)(
...
function loadPrompt(path, filename) {
  let name = (0, import_path.basename)(filename, ".prompt");
  let variant = null;
  if (name.includes(".")) {
